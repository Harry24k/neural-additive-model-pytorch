...
class Datasets():
    def __init__(self, data_name, root='./data',
        ...
        if val_info is not None:
            if label_filter is not None:
                raise ValueError(
                    "Validation + Label filtering is not supported yet.")
        ...
        if (data_name == "CIFAR10") or (data_name == "CIFAR100"):
            ...
            if train_transform is None:
                logging.warning(
                    "transforms.ToTensor() is used as a train transform.")
                train_transform = transforms.Compose([
                    transforms.ToTensor(),
                ])
            if test_transform is None:
                logging.warning(
                    "transforms.ToTensor() is used as a test transform.")
                test_transform = transforms.Compose([
                    transforms.ToTensor(),
                ])
            if val_transform is None:
                logging.warning(
                    "transforms.ToTensor() is used as a val transform.")
                val_transform = transforms.Compose([
                    transforms.ToTensor(),
                ])
        ...
        if set_data_method is not None:
            set_data_method(root, train_transform, test_transform)
        else:
        ...
        if corruption is not None:
            ...
            self.test_data = corrupt_cifar(
                root, data_name, self.test_data, corruption)
        ...
        if (self.val_info is not None) and (len(self.train_data) > 0):
            ...
            if isinstance(self.val_info, float):
                if self.val_info <= 0 or self.val_info >= 1:
                    ...
                    self.val_idx = np.random.RandomState(seed=self.val_seed).permutation(max_len)[
                        :self.val_len].tolist()
                ...
                raise ValueError(
                    "val_info must be the one of [int, float or list].")
            ...
            self.train_idx = list(
                set(range(len(self.train_data))) - set(self.val_idx))
            ...
            if data_name in ['ImageNet-O', 'ImageNet-A', 'ImageNet-R']:
                raise ValueError(
                    "Label filter is not supported for %s" % data_name)
    ...
    def _set_data_CIFAR10U(self, root, train_transform, test_transform):
        ...
        self.train_data = ConcatDataset(
            [self.train_data_sup, self.train_data_unsup])
    ...
    def _set_data_ImageNetO(self, root, train_transform, test_transform):
        self.train_data = EmptyDataset()
        self.test_data = get_imagnet_natural_adv(
            root, 'ImageNet-O', test_transform)

    def _set_data_ImageNetA(self, root, train_transform, test_transform):
        self.train_data = EmptyDataset()
        self.test_data = get_imagnet_natural_adv(
            root, 'ImageNet-A', test_transform)

    def _set_data_ImageNetR(self, root, train_transform, test_transform):
        self.train_data = EmptyDataset()
        self.test_data = get_imagnet_renditions(
            root, 'ImageNet-R', test_transform)
